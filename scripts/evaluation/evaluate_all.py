...
def evaluate_all(candidates, references, name):
    ...
    for c, r in zip(candidates, references):
        if c and r:
            ...
            valid_c.append(c)
            valid_r.append(r)
    P, R, F1 = bert_score(valid_c, valid_r, lang='ru', verbose=False)
    result = {
        'model': name,
...
print('\n--- Система 1: Lead-3 Russian ---')
r1 = evaluate_all(references, references, 'Lead-3 (ru)')

print('\n--- Система 2: TF-IDF + kazRush ---')
...
pd.DataFrame([r1, r2, r3, r4]).to_csv('full_evaluation_347.csv', index=False)
print('\nСохранено в full_evaluation_347.csv')
